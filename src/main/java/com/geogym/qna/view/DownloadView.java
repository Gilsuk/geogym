...
public class DownloadView extends AbstractView {
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
		...
		// 모델값 가져오기
		List<Attachment> file = (List<Attachment>) model.get("downFile");
...
//		ServletContext context = request.getSession().getServletContext();
			...
			String filename = file.get(i).getAttachment_stored_name();
			...
			String outputName = URLEncoder.encode(file.get(i).getAttachment_stored_name(), "utf-8");
			logger.info("변환된 파일명 : " + outputName);

			outputName = outputName.replace("+", "%20"); // 띄어쓰기 변환

			logger.info("보정된 파일명 : " + outputName);
			...
			// 서버에 저장된 파일 객체
			File origin = new File(context.getRealPath("upload"), file.get(i).getAttachment_stored_name());
//		File origin = src;
